/* ═══════════════════════════════════════════════════════
   ...
   Depends on: ai.js  supabase.js  getDB() (global)
═══════════════════════════════════════════════════════ */
...
const PT = (() => {
  ...
  function resetState(mod, key, user, questions, timeSecs) {
    S = {
      mod, key, user, questions,
      cur: 0, answers: {}, visited: new Set(), marked: new Set(),
      qTimers: {}, qStart: Date.now(),
      totalSecs:    timeSecs || 20 * 60,
      totalElapsed: 0, lastTick: Date.now(),
      paused: false, submitted: false, timerHandle: null,
  ...
  function injectStyles() {
    ...
    st.id = 'pt-styles';
    st.textContent = `
#pt-overlay{position:fixed;inset:0;z-index:500;background:var(--bg);display:flex;flex-direction:column;font-family:'DM Sans',sans-serif;}
.pt-hdr{display:flex;align-items:center;justify-content:space-between;padding:0 18px;height:52px;background:var(--surface);border-bottom:1px solid var(--border);flex-shrink:0;}
...
.pt-icon-btn{width:30px;height:30px;border-radius:50%;border:1px solid var(--border2);background:var(--bg2);color:var(--text2);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:14px;transition:all .15s;}
.pt-icon-btn:hover{background:var(--bg3);color:var(--text1);}
.pt-close-btn{padding:5px 14px;border-radius:var(--radius-sm);border:1px solid var(--border);background:transparent;font-size:13px;color:var(--text2);cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .15s;}
...
.pt-qpanel{flex:1;display:flex;flex-direction:column;overflow:hidden;min-width:0;}
.pt-qhdr{display:flex;align-items:center;justify-content:space-between;padding:11px 22px;border-bottom:1px solid var(--border);flex-shrink:0;}
...
.pt-opt-key{width:24px;height:24px;border-radius:5px;flex-shrink:0;background:var(--bg2);border:1px solid var(--border2);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;color:var(--text2);transition:all .13s;}
...
.pt-footer{padding:10px 20px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;background:var(--surface);flex-shrink:0;}
.pt-foot-l,.pt-foot-r{display:flex;gap:8px;}
.pt-btn{display:flex;align-items:center;gap:6px;padding:8px 15px;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--bg2);color:var(--text2);font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;cursor:pointer;transition:all .13s;}
...
.pt-palette{width:192px;flex-shrink:0;background:var(--surface);border-left:1px solid var(--border);display:flex;flex-direction:column;overflow:hidden;}
.pt-pal-hdr{padding:10px 13px;border-bottom:1px solid var(--border);font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;letter-spacing:.6px;flex-shrink:0;}
...
.pt-dot{aspect-ratio:1;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:500;cursor:pointer;border:1px solid var(--border2);background:var(--bg2);color:var(--text3);transition:all .12s;}
...
.pt-pal-legend{padding:8px 12px;border-top:1px solid var(--border);display:flex;flex-direction:column;gap:5px;flex-shrink:0;}
...
.pt-submit-btn{width:100%;padding:10px;background:var(--accent);color:#fff;border:none;border-radius:var(--radius-sm);font-family:'Instrument Serif',serif;font-size:15px;cursor:pointer;transition:opacity .15s;}
.pt-submit-btn:hover{opacity:.88;}
.pt-pal-note{font-size:11px;color:var(--text3);text-align:center;margin-top:5px;}
/* loading */
.pt-loading-screen{position:absolute;inset:0;background:var(--bg);z-index:10;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;}
.pt-spinner{width:36px;height:36px;border:2.5px solid var(--border);border-top-color:var(--accent);border-radius:50%;animation:pt-spin .8s linear infinite;}
...
.pt-load-bar-wrap{width:240px;height:5px;background:var(--border);border-radius:3px;overflow:hidden;margin-top:4px;}
.pt-load-bar{height:100%;background:var(--accent);border-radius:3px;transition:width .4s ease;width:0%;}
.pt-load-pct{font-size:12px;color:var(--text3);margin-top:2px;}
.pt-load-steps{display:flex;flex-direction:column;align-items:center;gap:4px;margin-top:4px;}
.pt-load-step{font-size:12px;color:var(--text3);display:flex;align-items:center;gap:6px;}
.pt-load-step.done{color:var(--green);}
.pt-load-step.active{color:var(--text1);font-weight:500;}
.pt-load-step-dot{width:6px;height:6px;border-radius:50%;background:var(--border2);flex-shrink:0;}
.pt-load-step.done .pt-load-step-dot{background:var(--green);}
.pt-load-step.active .pt-load-step-dot{background:var(--accent);}
.pt-load-error{display:none;flex-direction:column;align-items:center;gap:12px;text-align:center;padding:0 24px;}
...
.pt-load-retry-btn{padding:8px 20px;border-radius:var(--radius-sm);background:var(--accent);color:#fff;border:none;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;cursor:pointer;transition:opacity .15s;}
.pt-load-retry-btn:hover{opacity:.85;}
/* results */
.pt-results{position:absolute;inset:0;background:var(--bg);z-index:20;display:none;flex-direction:column;overflow-y:auto;}
.pt-results.show{display:flex;}
.pt-res-hdr{padding:18px 26px;background:var(--surface);border-bottom:1px solid var(--border);flex-shrink:0;display:flex;align-items:flex-start;justify-content:space-between;}
...
.pt-score-hero{display:flex;align-items:center;gap:20px;padding:18px 20px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:20px;}
.pt-score-ring{width:80px;height:80px;border-radius:50%;border:3px solid var(--border2);display:flex;align-items:center;justify-content:center;font-family:'Instrument Serif',serif;font-size:20px;color:var(--text1);flex-shrink:0;}
...
.pt-rv-item{padding:13px 16px;border-radius:var(--radius-sm);border:1px solid var(--border);background:var(--surface);border-left-width:3px;}
...
.pt-rv-fb{margin-top:8px;padding:8px 11px;background:var(--bg2);border-radius:var(--radius-sm);font-size:12px;color:var(--text2);line-height:1.55;border-left:2px solid var(--accent);}
@media(max-width:600px){.pt-palette{display:none;}.pt-score-stats{grid-template-columns:repeat(2,1fr);}}
    `;
  ...
  function buildOverlay() {
    ...
    div.innerHTML = `
      <div class="pt-loading-screen" id="pt-loading">
        ...
        <div class="pt-load-bar-wrap" id="pt-bar-wrap"><div class="pt-load-bar" id="pt-load-bar"></div></div>
        <div class="pt-load-pct" id="pt-load-pct">0%</div>
        <div class="pt-load-steps" id="pt-load-steps">
          <div class="pt-load-step active" id="pls-prompt"><div class="pt-load-step-dot"></div>Building prompt</div>
          <div class="pt-load-step"        id="pls-ai">    <div class="pt-load-step-dot"></div>Asking AI model</div>
          <div class="pt-load-step"        id="pls-parse"> <div class="pt-load-step-dot"></div>Parsing questions</div>
          <div class="pt-load-step"        id="pls-cache"> <div class="pt-load-step-dot"></div>Saving to cache</div>
        </div>
        ...
        <div class="pt-hdr-right">
          <div class="pt-clock-pair">
            ...
            <div class="pt-clock">
              ...
              <span class="pt-clock-val" id="pt-totalclock">--:--</span>
            </div>
          </div>
  ...
  /* ── LOAD STEP TRACKER ── */
  const STEP_IDS  = ['pls-prompt','pls-ai','pls-parse','pls-cache'];
  const STEP_PCTS = { 'pls-prompt':10, 'pls-ai':40, 'pls-parse':75, 'pls-cache':100 };
  ...
  function showLoadError(title, sub, retryFn) {
    ['pt-spinner','pt-load-steps','pt-bar-wrap','pt-load-pct'].forEach(id => {
      const el = document.getElementById(id); if (el) el.style.display = 'none';
    });
    document.getElementById('pt-err-title').textContent = title;
    document.getElementById('pt-err-sub').textContent   = sub;
    document.getElementById('pt-load-error').classList.add('show');
    const rb = document.getElementById('pt-retry-btn');
    if (rb && retryFn) rb.onclick = retryFn;
  }
  ...
  function resetLoadUI() {
    ...
    document.getElementById('pt-load-error').classList.remove('show');
  ...
  function dotCls(qi) {
    ...
    if (S.answers[qi] !== undefined) return c + (S.marked.has(qi) ? ' pt-attempted pt-marked' : ' pt-attempted');
    if (S.marked.has(qi))  return c + ' pt-marked';
    if (S.visited.has(qi)) return c + ' pt-seen';
    return c;
  }

  function updateDot(qi) { const el = document.getElementById('pt-dot-' + qi); if (el) el.className = dotCls(qi); }
  ...
  function renderQ(qi) {
    ...
    S.qStart = Date.now();
    const el = document.getElementById('pt-qtimer-live'); if (el) el.textContent = '0s';
    updateDot(qi);
  }
  ...
  async function generateQuestions(mod, key, user) {
    ...
    const entry = db?.[key];
    if (!entry) {
      showLoadError('Content not found', `Could not load content for "${key}".`, null);
      return null;
    }
    ...
    document.getElementById('pt-load-sub').textContent =
      `Using ${AI_CFG[getActiveModel()]?.label || 'AI'}…`;
    ...
    await generateAndCache({
      ...
      onReady: (generated, source) => {
        ...
        setLoadStep('pls-cache', source === 'cache' ? 'done' : 'active');
        qs = generated;
        if (source !== 'cache') setTimeout(() => setLoadStep('pls-cache', 'done'), 400);
      },
      onError: (err) => { genError = err; }
    });

    if (genError || !qs || !qs.length) {
      const isRL = String(genError?.status || genError).includes('429');
      showLoadError(
        isRL ? 'Rate limit reached' : 'Generation failed',
        isRL
          ? 'The AI model is busy. Switch to a different model in the nav, or wait 30 seconds and retry.'
          : 'AI couldn\'t generate questions. Check your connection and try again.',
        () => {
          resetLoadUI();
          generateQuestions(mod, key, user).then(q => { if (q) startTest(q, mod, key); });
        }
      );
      return null;
    }
  ...
  /* ══════════════ PUBLIC API ══════════════ */
  return {
    async open({ mod, key, user, timeSecs }) {
      injectStyles();
      const old = document.getElementById('pt-overlay'); if (old) old.remove();
      buildOverlay();
      resetState(mod, key, user, [], timeSecs);
    ...
    saveNext() { saveQTimer(); if (S.cur < S.questions.length - 1) goTo(S.cur + 1); else this.confirmSubmit(); },
    markNext() {
      saveQTimer();
    ...
    clearResp() { delete S.answers[S.cur]; renderQ(S.cur); updatePalNote(); },
    confirmSubmit() {
      const left = S.questions.length - Object.keys(S.answers).length;
    ...
    close() {
      clearInterval(S.timerHandle);
      const ov = document.getElementById('pt-overlay'); if (ov) ov.remove();
    }
  };
})();